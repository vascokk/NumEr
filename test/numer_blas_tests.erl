...
mmul() ->
    ...
    %% GPU CUBLAS test
    {ok, Buf_M1} = numer_nifs:new_matrix_float_buffer(M1, ?ROW_MAJOR),
    {ok, Buf_M2} =  numer_nifs:new_matrix_float_buffer(M2, ?ROW_MAJOR),
...
gemm_test()->
    ...
    {ok, Buf_C} = numer_buffer:new(matrix, float, row_major,_m,_n),
...
transpose_test()->
    ...
    {ok, Buf_M} = numer_buffer:new(matrix, float, row_major, M),
