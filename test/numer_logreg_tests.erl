...
cost_buf_test() ->
	...
	X = [ lists:append([1.0], lists:sublist(TrEx, 2)) || TrEx<-TrainSet], %[1.0] - adding the bias term
	Y = lists:append( [ lists:sublist(TrEx, 3,3) || TrEx<-TrainSet] ),  
	{ok, X_buf} = numer_buffer:new(matrix, float, row_major, X),
	{ok, Y_buf} = numer_buffer:new(matrix, float, row_major, [Y]), 
    {ok, T_buf} = numer_buffer:new(matrix, float, row_major, Theta),
	Cost = fennec_logreg:cost_buf(Ctx, T_buf, X_buf, Y_buf, length(hd(X)), length(X)),	
	numer_buffer:destroy(T_buf),
	numer_buffer:destroy(Y_buf),
	numer_buffer:destroy(X_buf),
	numer_context:destroy(Ctx),
...
gradient_buf_test() ->
	...
	{ok, Y_buf} = numer_buffer:new(matrix, float, row_major, [Y]), 
    {ok, T_buf} = numer_buffer:new(matrix, float, row_major, Theta),
	Grad_buf = fennec_logreg:gradient_descent_buf(Ctx, T_buf, X_buf, Y_buf, length(hd(X)), length(X)),	
	{ok, Grad} = numer_buffer:read(Grad_buf),
	numer_buffer:destroy(Grad_buf),
	numer_buffer:destroy(T_buf),
	numer_buffer:destroy(Y_buf),
	numer_buffer:destroy(X_buf),
